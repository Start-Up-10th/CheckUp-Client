...
| 2026-09-26 | 학생 공지 목록(REQ-COM-004)이 기능명세서·Figma에 없고 인터뷰 한 줄에서만 나온 것을 확인 → 팀원 의견으로 사용자가 학생 공지 목록 화면을 없애고 공지를 웹 내부 알림으로만 전달하기로 결정(#40). REQ-COM-004·REQ-UI-004·ACC-COM-004·DEC-012·SRC-CORRECTIONS 갱신, `/notice` 준비 중 틀 삭제. 관리자 공지 작성은 알림 발송 수단으로 유지(관리자 담당) | `npm run harness:check`, `web/` `npm run check` |
