...
| 2026-09-26 | 공지 기능(구 REQ-COM-004: 관리자 공지 CRUD·학생 공지 목록)이 기능명세서·Figma에 없고 인터뷰 한 줄에서만 나온 것을 확인 → 팀원 의견으로 사용자가 공지 기능 전체를 없애기로 결정하고, 동의 화면 `기숙사 공지 알림 수신`과 `기숙사 공지사항 등록` 알림도 함께 제거(#40). REQ-COM-004 삭제 후 REQ-SCOPE-003 제외 목록에 기록, REQ-COM-005(알림 2종)·REQ-AUTH-004(동의 필수 2개)·REQ-UI-004/005·architecture·operations·contracts·ACC 시나리오·DEC-009/012·SRC-CORRECTIONS 갱신. 웹은 `/notice` 틀 삭제, 동의 화면 선택 항목과 알림 mock의 공지 항목 제거 | `npm run harness:check`, `web/` `npm run check` |
...
학생 로그인 화면(`/login`)은 사용자 결정(2026-09-26)으로 UI를 먼저 만들고 버튼 동작만 OAuth 연동 때 연결한다(#32).
