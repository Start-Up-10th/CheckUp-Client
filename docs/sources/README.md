...
| SRC-NOTICE-AS-NOTIFICATION | 2026-09-26 대화: 학생 공지 목록이 기능명세서(student-ui/admin-ui)·Figma에 없고 인터뷰 한 줄에서만 나온 것을 확인한 뒤, 사용자가 팀원 의견으로 "그냥 알림일듯", "우리 알림 기능 있잖아"라고 결정 | REQ-COM-004, REQ-UI-004 |
...
| 학생은 공지 목록을 읽기만 한다, 알림 미동의 학생도 공지 목록 조회 가능(REQ-COM-004 이전 문구) | 학생 공지 목록 화면 없음. 공지는 `기숙사 공지사항 등록` 알림으로만 전달하고 미동의 학생은 받지 않음(SRC-NOTICE-AS-NOTIFICATION, 2026-09-26) |
