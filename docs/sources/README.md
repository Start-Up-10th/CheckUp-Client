...
| SRC-VOLUNTEER-HISTORY | 2026-09-26 대화: 사용자가 로컬 화면을 보다가 "봉사 내역이 피그마에는 있는데 여기는 없다"고 지적 → 인터뷰 `학생은 자기 봉사 횟수만 확인`을 내역 제외로 해석했던 것을 확인하고, 사용자가 "피그마에 봉사 내역이 있잖아 넣어야 하는 거 아니냐"로 복구 결정. 이어 활동명은 "Figma대로 활동명 표시"(관리자 적립 때 활동명 기록)를 선택 같은 날 팀원(관리자 담당)이 "활동명은 예시이고 실제는 날짜만"이라고 정정해 활동명 표시·기록을 되돌림(기능명세서 활동명 문구는 팀원이 수정) | REQ-COM-002, REQ-COM-003, REQ-SCOPE-003 |
...
| 학생 봉사 활동 목록 제외, 학생은 누적 횟수만 조회(REQ-COM-003 이전 문구) | 학생은 본인 누적 횟수와 본인 활동 내역(날짜·횟수)을 조회; Figma의 활동명은 예시라 표시·기록하지 않음(SRC-VOLUNTEER-HISTORY, 2026-09-26). 관리자 +1/-1 조정은 SRC-NOTION-CHECKUPZIP에 따라 허용 |
